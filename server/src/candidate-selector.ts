 ...
 */
function assignQuality(
  lossCp: number,
  thresholds: EloBand["qualityLossCp"],
): Quality {
...
function sampleQuality(
  targets: EloBand["qualityTargets"],
  rng: SeededRNG,
): PickQuality {
...
export class CandidateSelector {
  ...
  async selectMoves(
    ...
    globalLogger.info("select_start", {
      req: reqId,
      elo,
      nodes: band.nodesMain,
      moves: moves.length,
    });
    ...
    if (moves.length > 0) {
      playerPerformance = await this.warmupAndCalculateAccuracy(
        moves,
        playerColor,
      );
    }
    ...
    const refResult = await this.engine.analyzeNodes(
      positionCmd,
      band.nodesMain,
    );
    const currentFen = this.chess.fen();
    const sideToMove = currentFen.split(" ")[1] as "w" | "b";
    const bestEvalPlayerCp = toPlayerPov(
      refResult.evalCp,
      playerColor,
      sideToMove,
    );
    ...
    const legalSet = new Set(
      legal.map((m) => m.from + m.to + (m.promotion || "")),
    );
    ...
    for (const cand of candidates) {
      ...
      const promotion =
        cand.move.length > 4 ? cand.move[4].toLowerCase() : undefined;
      ...
      const evalResult = await this.engine.analyzeNodes(
        `position fen ${fenAfterMove}`,
        band.nodesCand,
      );
      const newSideToMove = fenAfterMove.split(" ")[1] as "w" | "b";
      cand.evalPlayerCp = toPlayerPov(
        evalResult.evalCp,
        playerColor,
        newSideToMove,
      );
    ...
    let accepted = candidates.filter(
      (c) => c.evalPlayerCp >= bestEvalPlayerCp - band.windowCp,
    );
    if (accepted.length < K) {
      accepted = [...candidates].sort(
        (a, b) => b.evalPlayerCp - a.evalPlayerCp,
      );
    }
    ...
    const pick1 =
      this.pickBucketed(accepted, band, rng, bestEvalPlayerCp) ??
      this.bestByEval(accepted);
    if (!pick1) {
      // No candidates at all (should not happen with K >= 1)
      return {
        lines: legal.slice(0, K).map((m) => ({
    ...
    const pick3 =
      K >= 3
        ? (this.pickBucketedWithSim(
            rem2,
            pick2 ? [pick1, pick2] : [pick1],
            band,
            rng,
            bestEvalPlayerCp,
            slot1Quality,
          ) ?? this.bestByEval(rem2))
        : undefined;

    globalLogger.info("select_sample", {
      req: reqId,
    ...
    const toVerify = [pick1, pick2, pick3].filter(
      Boolean,
    ) as BucketedCandidate[];
    ...
    for (const candidate of toVerify) {
      ...
      const promotion =
        candidate.move.length > 4 ? candidate.move[4].toLowerCase() : undefined;
      ...
      const verifyResult = await this.engine.analyzeNodes(
        `position fen ${fenAfterMove}`,
        VERIFY_NODES,
      );
      const newSideToMove = fenAfterMove.split(" ")[1] as "w" | "b";
      const verifyEvalPlayerCp = toPlayerPov(
        verifyResult.evalCp,
        playerColor,
        newSideToMove,
      );
      ...
      if (verifyEvalPlayerCp >= bestEvalPlayerCp - MAX_DROP_CP) {
        ...
        const isBrilliant =
          allowBrilliant && candidate.materialDrop >= 3 && verifyLossCp <= 30;
        verified.push({
          ...candidate,
        ...
        globalLogger.info("select_verify", {
          req: reqId,
          move: candidate.move,
    ...
    let result = [...verified];
    const acceptedRanked = [...accepted].sort(
      (a, b) => b.evalPlayerCp - a.evalPlayerCp,
    );
    ...
    if (legal.length >= 3 && result.length < 3) {
      globalLogger.error(
        "select_error",
        "Invariant broken: fewer than 3 moves",
        { req: reqId, legal: legal.length, result: result.length },
      );
    }
    ...
    globalLogger.info("select_result", {
      req: reqId,
      moves: result.map((x) => x.move).join(","),
    });
   ...
   */
  private buildCandidateFromMove(
    move: Move,
    playerColor: "w" | "b",
  ): BucketedCandidate {
    const uci = move.from + move.to + (move.promotion || "");
    ...
    const materialAfter = moveRes
      ? countMaterial(this.chess, playerColor)
      : materialBefore;
  ...
  private pickBucketed(
    ...
    for (let attempts = 0; attempts < 4; attempts++) {
      const bucket = pool.filter(
        (c) =>
          c.quality === targetQuality ||
          (c.quality === "bad" && targetQuality === "inaccuracy"),
      );
      if (bucket.length > 0) {
  ...
  private pickBucketedWithSim(
    ...
    let targetQuality = clampQuality(
      sampleQuality(band.qualityTargets, rng),
      slot1Quality,
    );

    for (let attempts = 0; attempts < 4; attempts++) {
      // Filter by quality
      let bucket = pool.filter(
        (c) =>
          c.quality === targetQuality ||
          (c.quality === "bad" && targetQuality === "inaccuracy"),
      );
      ...
      const filtered = bucket.filter(
        (c) => !selected.some((s) => this.tooSimilar(c, s, false)),
      );
      ...
      const hardFiltered = bucket.filter(
        (c) => !selected.some((s) => this.tooSimilar(c, s, true)),
      );
  ...
  private bestByEval(pool: BucketedCandidate[]): BucketedCandidate | undefined {
    ...
    return pool.reduce((best, c) =>
      c.evalPlayerCp > best.evalPlayerCp ? c : best,
    );
  }
  ...
  private async warmupAndCalculateAccuracy(
    ...
    console.log("startIndex", startIndex, moves.length);
    let totalAccuracy = 0;
    let movesAnalyzed = 0;
    let lastEvalCp = 0;
    ...
    const accuracy =
      movesAnalyzed > 0 ? Math.round(totalAccuracy / movesAnalyzed) : 0;
