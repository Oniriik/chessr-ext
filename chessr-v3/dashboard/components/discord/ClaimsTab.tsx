...
export function ClaimsTab() {
  ...
  const [path, setPath] = useState<'all' | 'paddle' | 'dashboard' | 'lifetime_set'>('all');
    ...
    <div className="space-y-3">
      <div className="flex flex-wrap items-center gap-2">
        <select value={path} onChange={(e) => setPath(e.target.value as typeof path)}
          ...
          <option value="lifetime_set">lifetime_set</option>
        </select>
        <form onSubmit={(e) => { e.preventDefault(); setOffset(0); load(); }} className="flex flex-1 sm:flex-none sm:w-64">
          <Input placeholder="Filter by claimer Discord ID…" value={search} onChange={(e) => setSearch(e.target.value)} className="h-8 text-[12px]" />
        </form>
      ...
      <Card>
        <CardContent className="p-0">
          {rows.length === 0 ? (
            <div className="p-6 text-center text-sm text-muted-foreground">{loading ? 'Loading…' : 'No claims.'}</div>
          ) : (
            <>
                ...
                <table className="w-full text-[12px]">
                  <thead className="border-b border-border/40 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">
                    <tr>
                      ...
                      <th className="px-3 py-2 text-left">Claimer</th>
                ...
                {rows.map((r) => (
                  <li key={r.id} className="space-y-1 p-3 text-[12px]">
                    ...
                    <div className="flex items-center justify-between gap-2 text-[10px] text-muted-foreground">
                      <PathBadge path={r.reward_path} />
                      <span>{timeAgo(r.claimed_at)}</span>
                    </div>
                  </li>
                ))}
              </ul>
