import { NextResponse } from 'next/server';
import { requireAdmin, isAdminContext } from '@/lib/auth-guard';

export const dynamic = 'force-dynamic';

// Super-admin only: apply a pending lifetime reward end-to-end. The
// serveur does the heavy lifting (cancel paddle, set lifetime, mark
// claimed). We just proxy + enforce the role gate.
export async function POST(req: Request) {
  const ctx = await requireAdmin(req);
  if (!isAdminContext(ctx)) return ctx;
  if (ctx.role !== 'super_admin') {
    return NextResponse.json({ error: 'super_admin required' }, { status: 403 });
  }

  const body = await req.json().catch(() => ({}));
  const rewardId = Number(body?.rewardId);
  if (!Number.isFinite(rewardId)) {
    return NextResponse.json({ error: 'rewardId required' }, { status: 400 });
  }

  const url = process.env.NEXT_PUBLIC_SERVEUR_URL || 'http://localhost:8081';
  const adminToken = process.env.SERVEUR_ADMIN_TOKEN || '';
  if (!adminToken) return NextResponse.json({ error: 'serveur token missing' }, { status: 500 });

  const res = await fetch(`${url}/admin/wheel/apply-lifetime`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'x-admin-token': adminToken },
    body: JSON.stringify({ rewardId, actorUserId: ctx.user.id }),
  });
  const data = await res.json().catch(() => ({}));
  return NextResponse.json(data, { status: res.status });
}
