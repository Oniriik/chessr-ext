...
// Super-admin only. Deletes a single unspun token. Spun tokens are
// untouchable — their reward might already be in someone's inventory.
export async function POST(req: Request) {
  ...
  const res = await fetch(`${url}/admin/wheel/token/revoke`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'x-admin-token': adminToken },
    body: JSON.stringify({ tokenId, actorUserId: ctx.user.id }),
  });
