...
#   ANALYTICS_DATABASE_URL="postgresql://chessr:<password>@localhost:5432/chessr_analytics" \
...
: "${ANALYTICS_DATABASE_URL:?ANALYTICS_DATABASE_URL not set}"
...
echo "--- Local ---"
psql "$ANALYTICS_DATABASE_URL" -c "SELECT user_id, event_type, created_at FROM user_activity ORDER BY created_at DESC LIMIT 3;"

echo
echo "==> Done. Next: flip USE_LOCAL_ANALYTICS=true in serveur/.env"
