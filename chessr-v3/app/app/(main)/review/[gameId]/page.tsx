...
export default function ReviewPage() {
  ...
  const analyzeGame = useCallback(async () => {
    ...
    try {
      ...
      if (!token || !userId) {
        setReviewError('Not signed in')
        setReviewLoading(false)
        return
      }

      // v3 serveur expects userId in the URL query — the auth handshake
      // below verifies the Supabase token resolves to the same id, plus
      // the shared APP_AUTH_TOKEN secret. Endpoint: /ws on api.chessr.io.
      const base = process.env.NEXT_PUBLIC_CHESSR_WS_URL || 'ws://localhost:8080'
      const wsUrl = `${base}/ws?userId=${encodeURIComponent(userId)}`
      const appToken = process.env.NEXT_PUBLIC_APP_AUTH_TOKEN
      const ws = new WebSocket(wsUrl)

      ws.onopen = () => {
        ws.send(JSON.stringify({ type: 'auth', token, source: 'app', appToken }))
      }
      ...
      ws.onmessage = (event) => {
        try {
          ...
          if (msg.type === 'auth_error') {
            setReviewError(`Auth failed: ${msg.error}`)
            setReviewLoading(false)
            ws.close()
            return
          }
