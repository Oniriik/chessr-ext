...
import { CheckCircle2, Shield, AlertTriangle, ChevronLeft, Search, Dna, Timer, ShieldCheck, Flag, Clock, Crown, Eye, EyeOff } from 'lucide-react'
  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        ...
        // v3 serveur expects userId in URL + auth message with appToken.
        const base = process.env.NEXT_PUBLIC_CHESSR_WS_URL || 'ws://localhost:8080'
        const wsUrl = `${base}/ws?userId=${encodeURIComponent(session.user.id)}`
