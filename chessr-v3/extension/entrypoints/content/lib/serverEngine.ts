 ...
 *
 * The server side is Komodo Dragon natif. Today Maia has no server-side
 * counterpart — if the caller asks for 'maia2' we still send the request
 * with engine='maia2', but the server will reply with suggestion_error
 * until we ship the native Maia binary. The UI surfaces that error.
 */
...
export class ServerEngine implements IEngine {
  ...
  async init(): Promise<void> {
    // WebSocket is already up (content.tsx connects it at auth time). Nothing
    // else needed — each `search()` is a standalone request/response exchange.
    this._ready = true;
  }
