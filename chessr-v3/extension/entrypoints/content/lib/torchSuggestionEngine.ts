/**
 * TorchSuggestionEngine — runs torch.wasm (Chess.com Explanation Engine)
 * in standard UCI suggestion mode (multi-PV search via `go`).
 *
 * Mirrors StockfishSuggestionEngine — same Worker / Blob URL / setoption
 * pack discovery / multi-PV labeling — but loads /engine/torch.{js,wasm}.
 *
 * Note: torch does NOT advertise Personality / UCI Elo / UCI LimitStrength
 * (they exist internally but are routed through the Chess.com proto pipeline,
 * not the public UCI surface). `buildEngineSetOptions` filters those out
 * automatically via the `_supportedOptions` set, so user-configured ELO /
 * Personality sliders are silently ignored when torch is selected.
 *
 * id = 'torch'.
 */
...
export interface TorchSuggestionSearchParams extends EngineParams {
  fen: string;
  moves?: string[];
  search?: SearchOptions;
}

export class TorchSuggestionEngine implements IEngine {
  readonly id = 'torch' as const;
  ...
  async init(): Promise<void> {
    const jsUrl = browser.runtime.getURL('/engine/torch.js');
    const wasmUrl = browser.runtime.getURL('/engine/torch.wasm');
    ...
    // Torch uses NNUE; no opening book here. Hash fragment carries the wasm URL.
  ...
  getCapabilities(): EngineCapabilities {
    ...
    return {
      hasPersonality: s.has('Personality'),                       // false on Torch
      hasUciElo: s.has('UCI Elo') || s.has('UCI_Elo'),             // false on Torch (proto-routed)
      hasDynamism: s.has('Dynamism'),                              // false on Torch
      hasKingSafety: s.has('King Safety'),                         // false on Torch
      hasVariety: s.has('Variety'),                                // false on Torch
    };
  }

  async search(params: IEngineSearchParams): Promise<LabeledSuggestion[]> {
    if (!this.worker || !this._ready) throw new Error('TorchSuggestionEngine not initialised');
    ...
    return new Promise<LabeledSuggestion[]>((resolve, reject) => {
      ...
      console.log('[Chessr][dbg][torch] setoption payload', opts);
      ...
      this.waitForToken('readyok').then(() => {
        ...
        this.sendRaw(buildGoCommand(params.search ?? null, 'torch'));
      });
    });
  }
  ...
  private onSearchLine(line: string) {
    ...
    if (line.startsWith('info') && line.includes(' pv ')) {
      ...
      // eval so the arrow still renders. Less common on torch than on
