import { ExternalLink } from 'lucide-react';
import { useAuthStore } from '../../stores/authStore';
import { useDiscordStore } from '../../stores/discordStore';
import { webSocketManager } from '../../lib/webSocket';
import { getRealHref } from '../../content/anonymousBlur';

const DISCORD_INVITE_URL = 'https://discord.gg/72j4dUadTu';

function DiscordIcon({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="currentColor">
      <path d="M20.317 4.37a19.791 19.791 0 0 0-4.885-1.515.074.074 0 0 0-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 0 0-5.487 0 12.64 12.64 0 0 0-.617-1.25.077.077 0 0 0-.079-.037A19.736 19.736 0 0 0 3.677 4.37a.07.07 0 0 0-.032.027C.533 9.046-.32 13.58.099 18.057a.082.082 0 0 0 .031.057 19.9 19.9 0 0 0 5.993 3.03.078.078 0 0 0 .084-.028 14.09 14.09 0 0 0 1.226-1.994.076.076 0 0 0-.041-.106 13.107 13.107 0 0 1-1.872-.892.077.077 0 0 1-.008-.128 10.2 10.2 0 0 0 .372-.292.074.074 0 0 1 .077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 0 1 .078.01c.12.098.246.198.373.292a.077.077 0 0 1-.006.127 12.299 12.299 0 0 1-1.873.892.077.077 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.839 19.839 0 0 0 6.002-3.03.077.077 0 0 0 .032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.03zM8.02 15.33c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.956-2.419 2.157-2.419 1.21 0 2.176 1.095 2.157 2.42 0 1.333-.956 2.418-2.157 2.418zm7.975 0c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.955-2.419 2.157-2.419 1.21 0 2.176 1.095 2.157 2.42 0 1.333-.946 2.418-2.157 2.418z" />
    </svg>
  );
}

export function DiscordLinkBanner() {
  const plan = useAuthStore((s) => s.plan);
  const { isLinked, inGuild, freetrialUsed, isLinking, setLinking } = useDiscordStore();

  // Linked + in guild → no banner
  if (isLinked && inGuild) return null;

  // Linked but not in guild → show "Join our Discord"
  if (isLinked && !inGuild) {
    return (
      <a
        href={DISCORD_INVITE_URL}
        target="_blank"
        rel="noopener noreferrer"
        className="tw-flex tw-items-center tw-gap-3 tw-px-3 tw-py-2.5 tw-rounded-lg tw-bg-[#5865F2]/15 tw-border tw-border-[#5865F2]/30 tw-text-[#cbd5ff] tw-w-full tw-text-left tw-cursor-pointer hover:tw-bg-[#5865F2]/25 tw-transition-colors tw-no-underline"
      >
        <div className="tw-w-8 tw-h-8 tw-rounded-full tw-bg-[#5865F2]/20 tw-flex tw-items-center tw-justify-center tw-flex-shrink-0">
          <DiscordIcon className="tw-w-4.5 tw-h-4.5 tw-text-[#5865F2]" />
        </div>
        <div className="tw-flex-1 tw-min-w-0">
          <p className="tw-text-xs tw-font-semibold">Join our Discord</p>
          <p className="tw-text-[10px] tw-text-[#8b95d6] tw-leading-tight">Get real-time updates and help from the community</p>
        </div>
        <ExternalLink className="tw-w-3.5 tw-h-3.5 tw-text-[#5865F2]/60 tw-flex-shrink-0" />
      </a>
    );
  }

  // Not linked → show "Link Discord" with contextual message
  const title = plan === 'free' && !freetrialUsed
    ? 'Get 3 days free'
    : 'Link Discord';
  const subtitle = plan === 'free' && !freetrialUsed
    ? 'Connect your Discord to start your free trial'
    : 'Connect your Discord account';

  const handleClick = () => {
    if (isLinking) return;
    setLinking(true);
    webSocketManager.send({
      type: 'init_discord_link',
      returnUrl: getRealHref(),
    });
  };

  return (
    <button
      onClick={handleClick}
      disabled={isLinking}
      className="tw-flex tw-items-center tw-gap-3 tw-px-3 tw-py-2.5 tw-rounded-lg tw-bg-[#5865F2]/15 tw-border tw-border-[#5865F2]/30 tw-text-[#cbd5ff] tw-w-full tw-text-left tw-cursor-pointer hover:tw-bg-[#5865F2]/25 tw-transition-colors disabled:tw-opacity-50"
    >
      <div className="tw-w-8 tw-h-8 tw-rounded-full tw-bg-[#5865F2]/20 tw-flex tw-items-center tw-justify-center tw-flex-shrink-0">
        <DiscordIcon className="tw-w-4.5 tw-h-4.5 tw-text-[#5865F2]" />
      </div>
      <div className="tw-flex-1 tw-min-w-0">
        {isLinking ? (
          <p className="tw-text-xs tw-font-medium">Redirecting to Discord...</p>
        ) : (
          <>
            <p className="tw-text-xs tw-font-semibold">{title}</p>
            <p className="tw-text-[10px] tw-text-[#8b95d6] tw-leading-tight">{subtitle}</p>
          </>
        )}
      </div>
    </button>
  );
}
