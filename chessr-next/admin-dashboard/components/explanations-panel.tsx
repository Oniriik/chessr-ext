...
import { RefreshCw, Sparkles, Users, TrendingUp, Calendar, MessageSquare } from 'lucide-react'
import {
  AreaChart,
  Area,
  XAxis,
...
interface ExplanationsResponse {
  ...
  timeline: {
    explanations: { time: string; count: number }[]
...
export function ExplanationsPanel() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const url = isCustom
        ? `/api/explanations?from=${encodeURIComponent(dateFrom)}&to=${encodeURIComponent(dateTo)}`
        : `/api/explanations?period=${period}`

      const res = await fetch(url)
      if (res.ok) setData(await res.json())
    } catch (err) {
      console.error('Failed to fetch explanations data:', err)
    } finally {
      setLoading(false)
    }
  ...
  const statCards = [
    ...
    {
      title: 'Explanations',
      value: data?.period.count?.toLocaleString() ?? '---',
    ...
    {
      title: 'Active Users',
      value: data?.period.uniqueUsers?.toLocaleString() ?? '---',
      description: 'Users who used explanations',
      icon: Users,
      color: 'text-cyan-400',
  ...
  const explanationsData = data?.timeline.explanations.map((d) => ({
    ...d,
    ...
    <div className="space-y-6">
      ...
      <Card className="border-border/50 bg-card/50 backdrop-blur">
        <CardHeader>
          ...
          <CardDescription>Move explanations over time</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="h-[300px]">
            {explanationsData.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={explanationsData}>
                  <defs>
                    <linearGradient id="gradExplanations" x1="0" y1="0" x2="0" y2="1">
      ...
      <Card className="border-border/50 bg-card/50 backdrop-blur">
        <CardHeader>
          ...
          <CardDescription>Unique users who used explanations per time bucket</CardDescription>
        </CardHeader>
        <CardContent>
            ...
            {activeUsersData.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={activeUsersData}>
                  <defs>
                    <linearGradient id="gradExplUsers" x1="0" y1="0" x2="0" y2="1">
      ...
      <Card className="border-border/50 bg-card/50 backdrop-blur">
        <CardHeader>
          ...
          <CardDescription>Users with the most explanations in this period</CardDescription>
        </CardHeader>
        <CardContent>
          {filteredTopUsers.length > 0 ? (
            ...
            <p className="text-sm text-muted-foreground">
              {discordOnly ? 'No Discord-linked users with explanations in this period' : 'No explanations in this period'}
            </p>
          )}
        </CardContent>
      </Card>
